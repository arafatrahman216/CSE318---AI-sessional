...
class Grid{
    public :
        ...
        bool isSolved(){
            ...
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    ...
                    if (boardConfiguration[i][j] != i * n + j + 1) {
                        return false;
                    }
                }
            }
        ...
        bool operator<(const Grid& other) const {
            ...
            return boardConfiguration < other.boardConfiguration;
        }
...
float findEuclideanDistance(Grid g){
...
float findlinearConflictHeuristic(Grid g){
    float manhattanDistance = findManhattanDistance(g);
    float linearConflict = LinearConflict(g);
...
void aStarSearch(Grid &startGrid, float (*heuristicFunction)(Grid) = findManhattanDistance){
    ...
    map <Grid , bool> closedList;
    closedList[startGrid] = true;
    ...
    while (!pq.empty()){
        ...
        if (currentGrid.isSolved()){
            ...
            cout << "Minimum number of moves = " << moves << "\n" << endl;
        ...
        for (auto move : validMoves){
            if (closedList[move]) continue; 
            exploredNodes++;
            float heuristic_Hn = heuristicFunction(move);
